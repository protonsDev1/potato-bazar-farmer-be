...
export const updateFarmerSchema = Joi.object({
  ...
  landDetails: Joi.array().items(Joi.object({
    landOwnedAcres: Joi.number().optional(),
    landLeasedAcres: Joi.number().optional(),
    totalLandUnderCultivation: Joi.number().optional(),
    landForPotatoFarming: Joi.number().optional(),
    areaUnderDrip: Joi.number().optional(),
    storageCapacityAtFarm: Joi.number().optional(),
    ...
    irrigationEquipmentBrand: Joi.string().optional().allow(null, ''),
    seedProcurementType: Joi.string().valid('new', 'reused', 'both').optional(),
    newSeedPercent: Joi.number().min(0).max(100).optional(),
    reusedSeedPercent: Joi.number().min(0).max(100).optional(),
    seedBrandName: Joi.string().optional().allow(null, ''),
    soilType: Joi.string().optional().allow(null, ''),
    averageYieldPerAcre: Joi.number().optional(),
    sowingMonth: Joi.string().optional().allow(null, ''),
  ...
  irrigationSources: Joi.array().items(
    Joi.object({ method: Joi.string().required() })
  ).optional(),

  irrigationMethods: Joi.array().items(
    Joi.object({ method: Joi.string().required() })
  ).optional(),

  potatoVarieties: Joi.array().items(
    Joi.object({
      variety: Joi.string().required(),
      subVariety: Joi.string().optional().allow(null, ''),
    })
  ...
  technologyUsed: Joi.array().items(
    Joi.object({ name: Joi.string().required() })
  ).optional(),

  sellingChannels: Joi.array().items(
    Joi.object({ name: Joi.string().required() })
  ).optional(),

  sellingChallenges: Joi.array().items(
    Joi.object({ name: Joi.string().required() })
  ).optional(),

  majorSellingChallenges: Joi.array().items(
    Joi.object({ name: Joi.string().required() })
  ).optional(),

  priceDiscoveryMethods: Joi.array().items(
    Joi.object({ method: Joi.string().required() })
  ).optional(),

  brandPreferenceReasons: Joi.array().items(
    Joi.object({ reason: Joi.string().required() })
  ).optional(),

  sellingPrices: Joi.array().items(
    Joi.object({ price: Joi.string().required() })
  ).optional(),
  ...
  sellingPlaces: Joi.array().items(
    Joi.object({ place: Joi.string().required() })
  ).optional(),

  potatoTypes: Joi.array().items(
